  ...
  <section>
    <div class="section-head">
      ...
      <div class="inline-actions">
        <RouterLink v-if="dictionary" class="btn primary" :to="{ path: '/objects', query: { dictionaryId: dictionary.id } }">
    ...
    <article class="card" :class="{ 'is-disabled': !canWrite }">
      ...
      <p class="muted">
        Одна таблица и редактирование через окно. Выберите строку и нажмите «Изменить», чтобы настроить правила.
      </p>
          ...
          <tbody>
            ...
            <tr v-for="row in schemaTableRows" :key="row.row_id">
              ...
              <td class="actions-row">
                <button class="btn btn-icon-only" title="Изменить атрибут схемы" :disabled="!canWrite || busy" @click="openEditEditor(row.row_id)">
                  <Pencil :size="16" aria-hidden="true" />
                  <span class="sr-only">Изменить атрибут схемы</span>
                </button>
                <button class="btn danger btn-icon-only" title="Удалить атрибут схемы" :disabled="!canWrite || busy" @click="removeSchemaRow(row.row_id)">
    ...
    <div v-if="editorOpen" class="modal-backdrop">
      <article class="modal-card">
        <div class="card-title-line">
          <h3>{{ editor.mode === 'create' ? 'Добавить атрибут в схему' : 'Редактирование атрибута схемы' }}</h3>
        ...
        <form class="form-grid" @submit.prevent="applyEditor">
          ...
          <div class="full check-stack">
            ...
            <label class="check">
              <input v-model="editor.is_unique" type="checkbox" />
              Уникальное значение
            </label>
          ...
          <label v-else class="full">
            ...
            <textarea
              v-if="editor.is_multivalue"
              v-model="editor.default_text"
          ...
          <label v-if="editorType === 'enum'" class="full">
            ...
            <textarea
              v-model="editor.allowed_values_text"
              class="code-area code-area-compact"
              placeholder="new&#10;used&#10;refurbished"
            ></textarea>
          </label>
